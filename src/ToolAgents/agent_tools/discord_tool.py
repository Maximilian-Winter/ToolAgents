...
class DiscordEmbedData(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="Description/content of the embed"
    )
    color: Optional[int] = Field(
        None, description="Color of the embed in decimal format (e.g. 3447003 for blue)"
    )
    ...
    thumbnail_url: Optional[str] = Field(
        None, description="URL of a thumbnail to include"
    )
...
class DiscordClient:
    ...
    def __init__(self, token: str, enable_privileged_intents: bool = False):
        ...
        if enable_privileged_intents:
            ...
            print(
                "WARNING: Using privileged intents. Make sure these are enabled in the Discord Developer Portal."
            )
    ...
    async def remove_reaction(self, channel_id: int, message_id: int, emoji: str):
        ...
        return {
            "success": True,
            "message": f"Removed reaction {emoji} from message {message_id}",
        }
...
class SendDiscordMessage(BaseModel):
    ...
    content: Optional[str] = Field(
        None, description="Content of the message if it's a text message"
    )
    embed_data: Optional[DiscordEmbedData] = Field(
        None, description="Embed data if message_type is embed"
    )
...
class GetDiscordChannels(BaseModel):
    ...
    def run(self, discord_client):
        ...
        return {"guild_id": self.guild_id, "channels": channels}
...
class CreateDiscordChannel(BaseModel):
    ...
    channel_type: str = Field(
        ..., description="Type of channel to create (text, voice, category)"
    )
    category_id: Optional[int] = Field(
        None, description="ID of the category to place the channel in"
    )
    topic: Optional[str] = Field(
        None, description="Topic/description for text channels"
    )
    ...
    def run(self, discord_client):
        ...
        return {"guild_id": self.guild_id, "channel": channel}
...
class AddDiscordReaction(BaseModel):
    ...
    emoji: str = Field(
        ..., description="Emoji to add as a reaction (Unicode emoji or custom emoji ID)"
    )
    ...
    def run(self, discord_client):
        ...
        result = loop.run_until_complete(
            discord_client.add_reaction(
                channel_id=self.channel_id, message_id=self.message_id, emoji=self.emoji
            )
        )
...
def init_discord_tools(enable_privileged_intents: bool = False):
    ...
    discord_client = get_discord_client(
        enable_privileged_intents=enable_privileged_intents
    )
    ...
    create_channel_tool = FunctionTool(
        CreateDiscordChannel, discord_client=discord_client
    )
    get_guild_info_tool = FunctionTool(
        GetDiscordGuildInfo, discord_client=discord_client
    )
