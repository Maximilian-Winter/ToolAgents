...
class WebSearchTool:
    def __init__(
        ...
        summarizing_api: ChatAPIProvider = None,
    ):
        self.web_crawler = web_crawler
        self.web_provider = web_provider
        if summarizing_api:
            self.text_transformer = TextTransformer(
                agent=ChatToolAgent(chat_api=summarizing_api),
                settings=summarizing_api.get_default_settings(),
                prompt_template=summarize_documents_based_on_document_type_and_user_query_message_template,
            )
        else:
        ...
        if self.text_transformer:
            ...
            for content in all_content:
                transformed_content.append(
                    self.text_transformer.transform(
                        document=content, document_type="Website", user_query=query
                    )
                )
