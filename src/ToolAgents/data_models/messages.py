...
class ContentType(str, Enum):
    ...
    Reasoning = "reasoning"  # thinking/reasoning blocks from reasoning models (Anthropic, OpenRouter)
...
class ContentBase(BaseModel):
    ...
    additional_fields: Dict[str, Any] = Field(
        default_factory=dict, description="Additional fields for the content."
    )
...
class ToolCallResultContent(ContentBase):
    ...
    tool_call_result_id: str = Field(
        ..., description="Unique identifier for the tool call result."
    )
    tool_call_id: str = Field(
        ..., description="Unique identifier for the corresponding tool call."
    )
    tool_call_name: str = Field(
        ..., description="The name of the tool that produced the result."
    )
...
class ReasoningContent(ContentBase):
    """
    Model for reasoning/thinking blocks produced by reasoning-capable models.

    These blocks contain the model's internal reasoning before its final response.
    They are separate from the visible text response and should be preserved
    for multi-turn conversations where supported.
    ...
    For Anthropic models (Claude 3.7+):
      ...
      - signature:  Anthropic's verification signature. Must be passed back
                    unchanged in subsequent requests to maintain reasoning continuity.
      - is_redacted: True when Anthropic's safety system has redacted the block.
                     In this case redacted_data holds the opaque payload that must
                     be passed back to the API unchanged.

    For OpenAI-compatible models via OpenRouter (e.g. MiMo-V2-Pro):
      - thinking:   The plaintext reasoning string from message.reasoning.
      - signature / redacted_data: Not used by these providers (None).
        ...
        for block in message.content:
            if isinstance(block, ReasoningContent):
                if block.is_redacted:
                    print("(redacted reasoning block)")
                else:
    ...
    Attributes:
        type: Always 'reasoning' for reasoning content.
        thinking: The plaintext reasoning content from the model.
        signature: Anthropic verification signature (must be preserved unchanged).
        is_redacted: True if this block was safety-filtered by Anthropic.
        redacted_data: Opaque payload for redacted blocks (must be preserved unchanged).
    """

    type: ContentType = Field(
        default=ContentType.Reasoning,
        description="The content type, always 'reasoning' for ReasoningContent.",
    )
    thinking: Optional[str] = Field(
        default=None,
        description="The plaintext reasoning content. None for redacted blocks.",
    )
    signature: Optional[str] = Field(
        default=None,
        description="Anthropic verification signature. Must be passed back unchanged in multi-turn requests.",
    )
    is_redacted: bool = Field(
        default=False,
        description="True if this block was safety-redacted by Anthropic.",
    )
    redacted_data: Optional[str] = Field(
        default=None,
        description="Opaque payload for redacted blocks. Must be passed back unchanged.",
    )

    def to_anthropic_block(self) -> Dict[str, Any]:
        """
        Serialise back to the Anthropic API content block format.
        Use this when passing reasoning blocks back in multi-turn requests.
        """
        if self.is_redacted:
...
class ChatMessage(BaseModel):
    ...
    content: List[
        Union[ReasoningContent, TextContent, BinaryContent, ToolCallContent, ToolCallResultContent]
    ] = Field(
    ...
    def contains_tool_call_results(self) -> bool:
        ...
        """
        return any(
            content.type == ContentType.ToolCallResult for content in self.content
        )

    def contains_reasoning(self) -> bool:
        """
        Returns True if the chat message contains any reasoning blocks.
        """
        return any(content.type == ContentType.Reasoning for content in self.content)
    ...
    def get_tool_calls(self) -> List[ToolCallContent]:
        ...
        """
        return [
            content for content in self.content if isinstance(content, ToolCallContent)
        ]
    ...
    def get_reasoning_blocks(self) -> List[ReasoningContent]:
        """
        Returns a list of ReasoningContent blocks from this message.
        """
        return [
            content for content in self.content if isinstance(content, ReasoningContent)
        ]

    def get_reasoning_text(self) -> Optional[str]:
        """
        Returns concatenated plaintext from all non-redacted reasoning blocks,
        or None if there are no reasoning blocks.
        """
        parts = [
            c.thinking for c in self.get_reasoning_blocks()
    ...
    def add_text_file_data(
        self, file: PathLike, content_prefix: str = "", content_suffix: str = ""
    ) -> None:
