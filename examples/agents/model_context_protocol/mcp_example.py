...
def setup_manual_mcp_tools():
    ...
    # Create the MCP tool
    weather_tool = MCPTool(tool_definition=weather_tool_def, debug_mode=True)
    ...
    # Create the MCP tool
    calculator_tool = MCPTool(tool_definition=calculator_tool_def, debug_mode=True)

    # Add these tools to a ToolRegistry
    MCPToolRegistry.add_mcp_tools_to_registry(
        tool_registry, [weather_tool, calculator_tool]
    )
    ...
    while True:
        ...
        if user_input.lower() == "exit":
            ...
            for tool in mcp_tools:
                print(
                    f"- {tool.tool_definition.name}: {tool.tool_definition.description}"
                )
            print()
        else:
            ...
            if "weather" in user_input.lower():
                ...
                print(
                    "The current weather in the location you asked about is 72°F and sunny."
                )
            elif any(
                term in user_input.lower()
                for term in ["calculate", "math", "compute", "+"]
            ):
                ...
                print(
                    "I don't have specific tools to help with that request, but I can answer general questions."
                )
