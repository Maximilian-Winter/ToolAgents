...
server_tool_registry.add_tools(
    [weather_tool, calculator_tool, add_note_tool, get_note_tool, list_notes_tool]
)
...
def initialize_mcp_client_agent(server_config):
    ...
    server_url = (
        f"http://{server_config.host}:{server_config.port}{server_config.prefix}"
    )
...
if __name__ == "__main__":
    ...
    print("----------------------------------------")
    print(
        "This example shows how to create an MCP server and an agent that uses MCP tools."
    )
    ...
    try:
        ...
        agent, settings, tool_registry, chat_history = initialize_mcp_client_agent(
            server_config
        )
