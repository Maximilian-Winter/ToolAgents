...
def test_mcp_client():
    ...
    print("\nRetrieving the note...")
    response = requests.post(
        f"{base_url}/tools/GetNote",
        json={"parameters": {"note_id": note_id}}
    )
    ...
    response = requests.post(
        ...
        json={"parameters": {"note_id": note_id, "content": "Remember to buy milk and eggs"}}
    )
