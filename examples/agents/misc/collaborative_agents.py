...
class CollaborativeTask(BaseModel):
    ...
    agent_role: str = Field(
        ...,
        description="The specialized role this agent should take (e.g., 'critic', 'researcher', 'implementer')",
    )
...
class AgentFeedback(BaseModel):
    ...
    agent_response: str = Field(
        ..., description="The response from another agent to review"
    )
    feedback_type: str = Field(
        ..., description="Type of feedback (e.g., 'critique', 'suggestion', 'question')"
    )
...
class TaskSynthesis(BaseModel):
    ...
    agent_responses: List[str] = Field(
        ..., description="List of responses from different agents"
    )
...
specialist_api = AnthropicChatAPI(
